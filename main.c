...
/*   Updated: 2024/10/23 09:19:29 by mbouhia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ASSERT(ft_strequ("hello", "hello") == 1,
		"ft_strequ failed for equal strings");
	ASSERT(ft_strequ("hello", "world") == 0,
		"ft_strequ failed for different strings");
	ASSERT(ft_strequ("", "") == 1, "ft_strequ failed for empty strings");
	ASSERT(ft_strequ("a", "b") == 0,
		"ft_strequ failed for different single-char strings");
	return (0);
}
...
int	test_ft_strnew(void)
{
	...
	{
		ASSERT(str[i] == '\0',
			"ft_strnew failed to initialize with null characters");
	}
	free(str);
	return (0);
}
...
int	test_ft_bzero(void)
{
	...
	ASSERT(strcmp(str + 5, "6789") == 0,
		"ft_bzero modified bytes it shouldn't have");
	return (0);
}
...
int	test_ft_strlen(void)
{
	...
	ASSERT(ft_strlen("This is a test") == 14,
		"ft_strlen failed for \"This is a test\"");
	return (0);
}
...
int	test_ft_strdup(void)
{
	...
	ASSERT(strcmp(original, duplicate) == 0,
		"ft_strdup failed to create an exact copy");
	free(duplicate);
	return (0);
}
...
int	test_ft_memccpy(void)
{
	...
	result = ft_memccpy(dest, src, '\200', 12);
	ASSERT(result == dest + 5, "ft_memccpy failed to return (correct pointer"));
	ASSERT(memcmp(src, dest, 5) == 0,
		"ft_memccpy failed to copy correct bytes");
	return (0);
}
...
int	test_ft_memchr(void)
{
	...
	ASSERT(ft_memchr(str, 'Z', 13) == NULL,
		"ft_memchr found a character that doesn't exist");
	return (0);
}
...
int	test_ft_memcmp(void)
{
	...
	ASSERT(ft_memcmp(str1, str2, 5) < 0,
		"ft_memcmp failed for different strings");
	ASSERT(ft_memcmp(str1, str1, 5) == 0,
		"ft_memcmp failed for identical strings");
	return 0;
}
...
int	test_ft_memmove(void)
{
	...
	ASSERT(strcmp(str, "memmove can be very very useful.") == 0,
		"ft_memmove failed");
	return 0;
}
...
int	test_ft_strlcat(void)
{
	...
	result = ft_strlcat(dest, ", World!", 13);
	ASSERT(strcmp(dest, "Hello, World") == 0,
		"ft_strlcat failed to concatenate correctly");
...
int	test_ft_strncmp(void)
{
	ASSERT(ft_strncmp("abc", "abd", 2) == 0,
		"ft_strncmp failed for equal prefixes");
...
int	test_ft_strstr(void)
{
	...
	ASSERT(ft_strstr(haystack, "World") == haystack + 7,
		"ft_strstr failed to find substring");
	ASSERT(ft_strstr(haystack, "OpenAI") == NULL,
		"ft_strstr found a non-existent substring");
	return 0;
}
...
int	test_ft_strchr(void)
{
	...
	ASSERT(ft_strchr(str, 'Z') == NULL,
		"ft_strchr found a non-existent character");
	return 0;
}
...
int	test_ft_strrchr(void)
{
	...
	ASSERT(ft_strrchr(str, 'o') == str + 8,
		"ft_strrchr failed to find last 'o'");
	ASSERT(ft_strrchr(str, 'Z') == NULL,
		"ft_strrchr found a non-existent character");
	return 0;
}
